...
In the code above, on initial build of the profile screen, the application checks if the code received as a parameter to the profile screen is an authorization code or a refresh token, and dispatches necessary actions based on the check. This is done using a `FirstBuildDispatcher` widget from the `rebloc` package. Also in the `build()` method, the application checks if user details have been retrieved. If retrieved, the details will be accessed from the view model and displayed, otherwise a loading indicator is rendered.
...
```dart
/// To get the user details from userinfo API endpoint of identity provider
