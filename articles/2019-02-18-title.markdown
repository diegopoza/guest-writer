...
Throughout this article, you'll implement an application that uses social login (with GitHub and Google) and database login. Upon logging in, the application will fetch and render the user's profile details as seen in the screenshots below.
...
With database login and Google social login enabled by default, click on the **Connection > Social** menu on your dashboard menu. Then, toggle on **GitHub** as one of the enabled social connections. In the displayed dialog, use the default details and click the **SAVE** button to finish.

Finally, navigate to the **Settings** tab of your application to set a callback URI in the **Allowed Callback URLs** field. 
...
Before getting started, create the following directories and files in addition to the `main.dart` file in the project's `/lib` directory such that your `/lib` directory's structure is as seen below:
